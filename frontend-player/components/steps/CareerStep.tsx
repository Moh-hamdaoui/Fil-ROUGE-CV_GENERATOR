...
import { DIVISIONS, CATEGORIES, SEASONS } from '@/lib/constants'
import { ArrowRight, ArrowLeft, Briefcase, Plus, Trash2, Trophy, Star, Crown, Medal, Globe } from 'lucide-react'
...
export function CareerStep() {
  ...
  const onSubmit = (formData: CareerFormData) => {
    // Valider qu'au moins une saison est remplie
    const validCareers = formData.careers.filter(c => c.clubName && c.season)
    if (validCareers.length === 0) {
      alert('Veuillez ajouter au moins une saison')
      return
    }
    updateData({ careers: formData.careers })
    setStep(6)
  }
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select {...form.register(`careers.${index}.competition`)} className="w-full bg-[#0a0a0a] border border-[#404040] rounded-lg px-4 py-3 text-white focus:border-[#f59e0b] focus:outline-none">
                    <option value="">Choisir...</option>
                ...
                {form.watch(`careers.${index}.isInternationalPlayer`) && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <input
                      ...
                      placeholder="Équipe nationale (ex: France U19)"
                      className="w-full bg-[#0a0a0a] border border-[#404040] rounded-lg px-4 py-3 text-white focus:border-[#f59e0b] focus:outline-none"
                    />
                    <input
                      {...form.register(`careers.${index}.aboutInternationalSelection`)}
                      placeholder="Détails (ex: 5 sélections)"
                      className="w-full bg-[#0a0a0a] border border-[#404040] rounded-lg px-4 py-3 text-white focus:border-[#f59e0b] focus:outline-none"
                    />
                  </div>
                )}
